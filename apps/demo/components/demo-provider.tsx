...
export function DemoProvider({ children }: { children: ReactNode }) {
  ...
  const addToCart = useCallback((variantId: string, quantity = 1) => {
    ...
    toast.success("Added to your demo cart");
  }, [commit]);
  ...
  const reset = useCallback(() => { localStorage.removeItem(STORAGE_KEY); stateRef.current = initialState; setState(initialState); toast.success("Demo data restored"); }, []);
